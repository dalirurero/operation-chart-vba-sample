...
1. `sample/` フォルダ内のSENDファイルをそれぞれ開き、データの日付を更新し、保存します。
3. Excelで `OperationChart_Sample.xlsm` を開きます。
4. マクロが無効になっている場合は、マクロを有効にします。
5. `MF` シートの「入荷データ取込」ボタンから、納品データ・納品予定データを取り込みます。
6. 作業編成画面で担当者を設定し、必要に応じて指示書・作業編成表を印刷します。
